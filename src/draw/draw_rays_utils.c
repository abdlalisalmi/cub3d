...
/*   Updated: 2020/11/25 09:42:12 by aes-salm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void horizontal_wall_hint_calc(int colume_id)
{
    ...
    rnorm.Xinter_cept = data->px + (rnorm.Yinter_cept - data->py) / tan(rays[colume_id].ray_angle);
    rnorm.Ystep = SQUARE;
...
void    horizontal_wall_hint_find(int colume_id)
{
    ...
    while (rnorm.next_horz_x >= 0 && rnorm.next_horz_x < (SQUARE * file.num_cols) && 
            rnorm.next_horz_y >= 0 && rnorm.next_horz_y < (SQUARE * file.num_rows))
    {
        if (wall_check(rnorm.next_horz_x, rays[colume_id].ray_facing_up ? rnorm.next_horz_y - 1 : rnorm.next_horz_y))
        {
    ...
    ? distance_between_points(data->px, data->py, rnorm.wall_horz_hit_X, rnorm.wall_horz_hit_Y) 
    : INT_MAX;
}
...
void    vertical_wall_hint_calc(int colume_id)
{
    ...
    rnorm.Yinter_cept = data->py + (rnorm.Xinter_cept - data->px) * tan(rays[colume_id].ray_angle);
    rnorm.Xstep = SQUARE;
    ...
    rnorm.Ystep *= (rays[colume_id].ray_facing_down && rnorm.Ystep < 0) ? -1 : 1;
...
void    vertical_wall_hint_find(int colume_id)
{
    ...
    while (rnorm.next_vert_x >= 0 && rnorm.next_vert_x < (SQUARE * file.num_cols) && 
            rnorm.next_vert_y >= 0 && rnorm.next_vert_y < (SQUARE * file.num_rows))
    {
        if (wall_check(rays[colume_id].ray_facing_L ? rnorm.next_vert_x - 1: rnorm.next_vert_x, rnorm.next_vert_y))
        {
    ...
    ? distance_between_points(data->px, data->py, rnorm.wall_vert_hit_X, rnorm.wall_vert_hit_Y) 
    : INT_MAX;
}